...
For each matching package blazedock will execute the specified command in the package component's origin.
...
Example use:
  ...
  blazedock exec --filter-type yarn -- pwd

  # run go get in all Go packages
  blazedock exec --filter-type go -- go get -v ./...

  # list all Go packages in the workspace
  blazedock exec --filter-type go --raw-output -- echo {}

  # execute go build in all direct Go dependencies when any of the relevant source files changes:
  blazedock exec --package some/other:package --dependencies --filter-type go --parallel --watch -- go build

  # run tsc watch for all dependent yarn packages (once per component origin):
  blazedock exec --package some/other:package --transitive-dependencies --filter-type yarn --parallel -- tsc -w --preserveWatchOutput
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		var pkgs map[*blazedock.Package]struct{}
		if len(packages) == 0 {
			pkgs = make(map[*blazedock.Package]struct{}, len(ws.Packages))
			for _, p := range ws.Packages {
				pkgs[p] = struct{}{}
			}
		} else {
			pkgs = make(map[*blazedock.Package]struct{}, len(packages))
		...
		spkgs := make([]*blazedock.Package, 0, len(pkgs))
		for p := range pkgs {
			spkgs = append(spkgs, p)
		}
		blazedock.TopologicalSort(spkgs)
		...
		if watch {
			...
			evt, errs := blazedock.WatchSources(context.Background(), spkgs, 2*time.Second)
			for {
				select {
				case <-evt:
					err := executeCommandInLocations(args, locs, noExecCache{}, parallelism, rawOutput)
...
func (c filesystemExecCache) filename(loc commandExecLocation) (string, error) {
	...
	if loc.Package != nil {
		...
		id = blazedock.FilesystemSafeName(loc.Component.Name)
	} else if loc.Dir != "" {
		id = blazedock.FilesystemSafeName(loc.Dir)
	} else if loc.Name != "" {
		id = loc.Name
	}
