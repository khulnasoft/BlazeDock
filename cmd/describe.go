...
import (
	...
	"github.com/khulnasoft/blazedock/pkg/prettyprint"
)
...
func getTarget(args []string, findScript bool) (comp *blazedock.Component, pkg *blazedock.Package, script *blazedock.Script, exists bool) {
...
func absPackageName(workspace blazedock.Workspace, name string) string {
...
func newMetadataDescription(pkg *blazedock.Package) packageMetadataDescription {
...
func newPackageDesription(pkg *blazedock.Package) packageDescription {
...
func newConfigDescription(tpe blazedock.PackageType, c blazedock.PackageConfig) configDescription {
	cfg := make(configDescription)
	switch tpe {
	case blazedock.DockerPackage:
		c := c.(blazedock.DockerPkgConfig)
		cfg["buildArgs"] = c.BuildArgs
		cfg["dockerfile"] = c.Dockerfile
		cfg["image"] = c.Image
		cfg["squash"] = c.Squash
	case blazedock.GenericPackage:
		c := c.(blazedock.GenericPkgConfig)
		cfg["commands"] = c.Commands
		cfg["test"] = c.Test
		cfg["dontTest"] = c.DontTest
	case blazedock.GoPackage:
		c := c.(blazedock.GoPkgConfig)
	...
	case blazedock.YarnPackage:
		c := c.(blazedock.YarnPkgConfig)
		cfg["dontTest"] = c.DontTest
		cfg["packaging"] = c.Packaging
...
func describePackage(out *prettyprint.Writer, pkg *blazedock.Package) {
...
func newComponentDescription(comp *blazedock.Component) componentDescription {
...
func describeComponent(out *prettyprint.Writer, comp *blazedock.Component) {
	if out.Format == prettyprint.TemplateFormat && out.FormatString == "" {
...
func newScriptDescription(s *blazedock.Script) scriptDescription {
