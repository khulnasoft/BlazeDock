...
import (
	...
	"github.com/khulnasoft/blazedock/cmd"
	"github.com/khulnasoft/blazedock/pkg/blazedock"
	"gopkg.in/yaml.v3"
)

type Setup struct {
	Workspace  blazedock.Workspace  `yaml:"workspace"`
...
type Component struct {
	...
	Files    map[string]string `yaml:"files"`
	Comp     blazedock.Component  `yaml:"comp"`
	Packages []blazedock.Package  `yaml:"packages"`
	Scripts  []blazedock.Script   `yaml:"scripts"`
}
...
// Materialize produces a blazedock workspace according to the setup spec
func (s Setup) Materialize() (workspaceRoot string, err error) {
	workspaceRoot, err = os.MkdirTemp("", "blazedock-test-*")
	if err != nil {
		return
	}
	...
	for _, comp := range s.Components {
		...
		cmp := struct {
			Constants blazedock.Arguments `yaml:"const,omitempty"`
			Packages  []blazedock.Package `yaml:"packages,omitempty"`
			Scripts   []blazedock.Script  `yaml:"scripts,omitempty"`
		}{
...
func (ft *CommandFixtureTest) Run() {
	...
	ft.T.Run(ft.Name, func(t *testing.T) {
		...
		n := 0
		for _, x := range env {
			if strings.HasPrefix(x, "BLAZEDOCK_") {
				continue
			}
			env[n] = x
			n++
