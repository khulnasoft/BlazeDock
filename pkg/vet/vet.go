...
type checkFunc struct {
	...
	runPkg func(pkg *blazedock.Package) ([]Finding, error)
	runCmp func(pkg *blazedock.Component) ([]Finding, error)
}
...
func (cf *checkFunc) Init(blazedock.Workspace) error {
	return nil
}

func (cf *checkFunc) RunPkg(pkg *blazedock.Package) ([]Finding, error) {
	if cf.runPkg == nil {
...
func (cf *checkFunc) RunCmp(pkg *blazedock.Component) ([]Finding, error) {
	if cf.runCmp == nil {
...
// PackageCheck produces a new check for a blazedock package
func PackageCheck(name, desc string, tpe blazedock.PackageType, chk func(pkg *blazedock.Package) ([]Finding, error)) Check {
	return &checkFunc{
		info: CheckInfo{
...
// ComponentCheck produces a new check for a blazedock component
func ComponentCheck(name, desc string, chk func(pkg *blazedock.Component) ([]Finding, error)) Check {
	return &checkFunc{
		info: CheckInfo{
...
type Check interface {
	...
	Init(ws blazedock.Workspace) error
	RunPkg(pkg *blazedock.Package) ([]Finding, error)
	RunCmp(pkg *blazedock.Component) ([]Finding, error)
}
...
func Run(workspace blazedock.Workspace, options ...RunOpt) ([]Finding, []error) {
	var opts runOptions
	...
	var (
		...
		runCompCheck = func(c Check, comp *blazedock.Component) {
			info := c.Info()
		...
		runPkgCheck = func(c Check, pkg *blazedock.Package) {
			info := c.Info()
			if !info.PackageCheck {
				return
			}
