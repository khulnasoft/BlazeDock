...
func init() {
	...
	register(PackageCheck("node-modules-source", "checks if the package's sources include node_modules/", blazedock.YarnPackage, checkYarnNodeModulesSource))
}
...
func (c *checkImplicitTransitiveDependencies) Init(ws blazedock.Workspace) error {
	c.pkgs = make(map[string][]string)
	for pn, p := range ws.Packages {
		if p.Type != blazedock.YarnPackage {
			continue
		}
...
func (c *checkImplicitTransitiveDependencies) getPkgJSON(pkg *blazedock.Package) (*pkgJSON, error) {
	var (
		found bool
...
func (c *checkImplicitTransitiveDependencies) RunCmp(pkg *blazedock.Component) ([]Finding, error) {
	return nil, fmt.Errorf("not a component check")
}

func (c *checkImplicitTransitiveDependencies) RunPkg(pkg *blazedock.Package) ([]Finding, error) {
	...
	for yarnDep, src := range depsInCode {
		var found bool
		for _, blazedockDep := range c.pkgs[yarnDep] {
			for _, dep := range pkg.GetDependencies() {
				if dep.FullName() == blazedockDep {
					found = true
					break
				}
			}
...
func checkYarnNodeModulesSource(pkg *blazedock.Package) ([]Finding, error) {
	var res []string
	for _, src := range pkg.Sources {
