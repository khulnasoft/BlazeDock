...
func TestResolveBuiltinVariables(t *testing.T) {
	tests := []struct {
		...
		{DockerPackage, DockerPkgConfig{Dockerfile: "blazedock.Dockerfile", Image: []string{"foobar:${__pkg_version}"}}, nil, DockerPkgConfig{Dockerfile: "blazedock.Dockerfile", Image: []string{"foobar:this-version"}}},
...
func TestCodecovComponentName(t *testing.T) {
	tests := []struct {
		...
		{"lower case", "COMPONENTS/devpod-cli:app", "components-devpod-cli-app-coverage.out"},
