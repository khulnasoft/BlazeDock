...
import (
	...
	"github.com/khulnasoft/blazedock/pkg/testutil"
)

// Used in multiple tests to verify how we deal with scripts having dependencies
var genericPackage = blazedock.Package{
	PackageInternal: blazedock.PackageInternal{
		Name:    "something",
		Type:    "generic",
		Sources: []string{"*.txt"},
	},
	Config: blazedock.GenericPkgConfig{
...
func TestScriptArgs(t *testing.T) {
	...
	setup := &testutil.Setup{
		Components: []testutil.Component{
			{
				Location: "scripts",
				Packages: []blazedock.Package{},
				Scripts: []blazedock.Script{
					{
						Name:        "echo",
	...
	tests := []*testutil.CommandFixtureTest{
		// If the argument isn't passed then Blazedock should fail with an exit code of 1
		{
			Name:              "unresolved arg",
			T:                 t,
...
func TestWorkingDirLayout(t *testing.T) {
	...
	setup := &testutil.Setup{
		Components: []testutil.Component{
			{
				Location: "scripts",
				Packages: []blazedock.Package{genericPackage},
				Scripts: []blazedock.Script{
					{
						Name:          "pwd-origin",
...
func TestScriptParallel(t *testing.T) {
	...
	tests := []*testutil.CommandFixtureTest{
		...
		// When one of the scripts fail, Blazedock should still run the other scripts to completions
