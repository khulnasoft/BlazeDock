...
func executeCommandsForPackageSafe(buildctx *buildContext, p *Package, wd string, commands [][]string) error {
	tmpdir, err := os.MkdirTemp("", "blazedock-*")
	if err != nil {
		return err
	}
	...
	spec.Mounts = append(spec.Mounts, specs.Mount{Destination: "/blazedock", Source: self, Type: "bind", Options: []string{"bind", "private"}})
	...
	spec.Process.Args = []string{"/blazedock", "plumbing", "exec", "/commands"}
	if log.IsLevelEnabled(log.DebugLevel) {
