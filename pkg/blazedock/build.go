...
const (
	...
	EnvvarCacheDir = "BLAZEDOCK_CACHE_DIR"

	// EnvvarBuildDir names the environment variable we take the build dir location from
	EnvvarBuildDir = "BLAZEDOCK_BUILD_DIR"

	// EnvvarYarnMutex configures the mutex flag blazedock will pass to yarn.
	// See https://yarnpkg.com/lang/en/docs/cli/#toc-concurrency-and-mutex for possible values.
	// Defaults to "network".
	EnvvarYarnMutex = "BLAZEDOCK_YARN_MUTEX"
...
func newBuildContext(options buildOptions) (ctx *buildContext, err error) {
	...
	if buildDir == "" {
		buildDir = filepath.Join(os.TempDir(), "blazedock", "build")
	}
	...
	defer self.Close()
	blazedockHash := sha256.New()
	_, err = io.Copy(blazedockHash, self)
	if err != nil {
		return nil, xerrors.Errorf("cannot compute hash of myself: %w", err)
	}
	...
	ctx = &buildContext{
		...
		blazedockHash:         hex.EncodeToString(blazedockHash.Sum(nil)),
	}
...
func (p *Package) packagesToDownload(inLocalCache map[*Package]struct{}, inRemoteCache map[*Package]struct{}, toDownload map[*Package]struct{}) {
	...
	// to perform a build.
	//
	// Example: components/ee/agent-smith:app depends on components/devpod-protocol/go:lib
	// 			components/devpod-protocol/go:lib depends on components/devpod-protocol:devpod-schema
	// 			To build components/ee/agent-smith:app it is not enough to just download components/devpod-protocol/go:lib
	// 			as we also need components/devpod-protocol:devpod-schema to be available on disk to perform the build.
...
func (p *Package) buildYarn(buildctx *buildContext, wd, result string) (bld *packageBuild, err error) {
	...
	if cfg.Packaging == YarnApp {
		...
		// The yarn package name and blazedock package name do not have to be the same which makes it possible to "reuse" the npm package name for
...
func parseGoCoverOutput(input string) (coverage, funcsWithoutTest, funcsWithTest int, err error) {
	...
	// github.com/khulnasoft/devpod/content_ws/pkg/contentws/contentws.go:33:	New		100.0%
...
func executeCommandsForPackage(buildctx *buildContext, p *Package, wd string, commands [][]string) error {
	...
	env = append(env, fmt.Sprintf("BLAZEDOCK_WORKSPACE_ROOT=%s", p.C.W.Origin))
